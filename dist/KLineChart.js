...
export function KLineChart({ chartSettingName, token, children, timeEndLoader, height = 300, enableRealTime = true, }) {
    ...
    return (_jsx(ChartSettingsContext.Provider, { value: settings, children: _jsx(ChartContext.Provider, { value: chartStore, children: _jsxs(Box, { display: "flex", flexDirection: "column", children: [_jsxs(Box, { ref: controlsRef, children: [_jsx(KLineMobile, {}), _jsxs(Stack, { direction: { xs: "column", sm: "row" }, spacing: { sx: 0, sm: 1 }, alignItems: { xs: "start", sm: "center" }, children: [_jsx(ToggleButtonGroup, { size: "small", sx: { height: 32 }, color: "primary", value: timeframe, exclusive: true, onChange: (event, newTF) => handleUpdateTimeframe(newTF), children: TIMEFRAMES.map(({ label, value }) => (_jsx(ToggleButton, { value: value, children: label }, value))) }), _jsxs(Stack, { direction: "row", spacing: 1, alignItems: "center", children: [_jsx(IndicatorSelector, { chart: chartStore, name: chartSettingName }), _jsx(KLineChartSettingsModal, { name: chartSettingName, onClose: handleRefreshSettings, variant: "position" }), _jsx(KLineChartSettingsModal, { name: chartSettingName, onClose: handleRefreshSettings, variant: "projection" }), token && (_jsx(KLineDataLoader, { timeframe: timeframe, tradeGroupId: token.default_trade_group_id, timeEndLoader: timeEndLoader, symbol: token.symbol, enableRealTime: enableRealTime }))] })] }), token && (_jsx(KLineProjection, { tokenName: token.symbol, symbolId: token.id, timeframe: timeframe, selectedTime: selectedTime, clearSelectedTime: handleClearSelectedTime })), children] }), _jsxs(Stack, { direction: "row", width: "100%", flex: 1, children: [_jsx(KLineChartSidePanel, {}), _jsx(Box, { ref: chartEl, height: chartHeight, width: "100%" })] })] }) }) }));
}
