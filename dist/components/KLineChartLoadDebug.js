...
export function KLineChartLoadDebug({ backtestRunId }) {
    const { t } = useTranslation();
    const chart = useChart();
    const [selectedTime, setSelectedTime] = useState();
    const [selectedPrice, setSelectedPrice] = useState();
    const { data: backtestRunTimeInfos, refetch: handleLoadDebug, isLoading: isLoadingDebug, isSuccess: isSuccessDebug, } = useBacktestRunDebug(backtestRunId);
    const timeInfo = selectedTime
        ? backtestRunTimeInfos === null || backtestRunTimeInfos === void 0 ? void 0 : backtestRunTimeInfos[selectedTime]
        : undefined;
    useEffect(() => {
        if (!chart) {
            return;
            ...
            switch (draw.variant) {
                case "Box":
                    chart.createOverlay({
                        ...
                        extendData: {
                            messages: (_a = draw.title) === null || _a === void 0 ? void 0 : _a.split("\n"),
                        },
                        points: [
                            {
                                timestamp: draw.start_time,
                                value: draw.direction === "LONG" ? draw.price_min : draw.price_max,
                            },
                            {
                                timestamp: draw.end_time,
                                value: draw.direction === "LONG" ? draw.price_max : draw.price_min,
                            },
                        ],
                    });
                    break;
    ...
    if (!backtestRunTimeInfos) {
        return (_jsx(LoadingButton, { size: "small", variant: "outlined", onClick: () => handleLoadDebug(), loading: isLoadingDebug, disabled: isSuccessDebug, children: t("load_debug") }));
    }
    return (_jsxs(Box, { height: "128px", children: [selectedTime && (_jsx(Typography, { variant: "body2", component: "p", children: format(selectedTime, DATETIME_UI) })), timeInfo === null || timeInfo === void 0 ? void 0 : timeInfo.messages.map((msg, idx) => (_jsx(Typography, { variant: "body2", component: "p", children: msg }, idx)))] }));
}
