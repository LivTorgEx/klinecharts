...
registerIndicator({
    ...
    calc: (kLineDataList, indicator) => {
        ...
        while (lastPoint && points.length > 5) {
            ...
            const isTrend = (lastPoint.isHigh &&
            ...
            const direction = firstPoint.price < secondPoint.price
                ? OrderDirection.LONG
    ...
    draw: ({ ctx, indicator, bounding, xAxis, yAxis, chart }) => {
        ...
        result
            .filter(({ coordinatePoints }) => coordinatePoints.some(({ index }) => index >= from && index <= to))
            .forEach((line) => {
