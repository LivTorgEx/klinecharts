...
export function KLineProjectionMessages({ tokenName }) {
    const ws = useContext(ClientWebSocketContext);
    const { timeframe } = useChartSettings();
    const [messages, setMessages] = useState([]);
    const handleUpdateProjection = useCallback((projection) => {
        var _a, _b, _c, _d;
        ...
        const asset_diff = formatBigNumber(projection.indicator.asset_diff_01 * 100);
        const messages = [
            [
                projection.status,
        ...
        if (projection.order_book) {
            ...
            if (ob.long_levels[0] && ob.short_levels[0]) {
                const spread = toMeasurePrice(ob.short_levels[0].price, ob.long_levels[0].price);
                orderBookMessage.push(`Spread[0]: ${spread.toFixed(2)}%`);
            }
            if (ob.long_levels[4] && ob.short_levels[4]) {
                const spread = toMeasurePrice(ob.short_levels[4].price, ob.long_levels[4].price);
        ...
        if (projection.oi) {
            ...
            const oiMessage = [
                ...
                `ChangeQTY: ${formatBigNumber(oi.change_qty, "")}|${formatBigNumber(oi.change_qty * ((_a = projection.price) !== null && _a !== void 0 ? _a : 0))}`,
                `Average: ${formatBigNumber(oi.avg * ((_b = projection.price) !== null && _b !== void 0 ? _b : 0))}`,
                `Candle: ${formatBigNumber((_d = (_c = projection.candle) === null || _c === void 0 ? void 0 : _c.qtym_asset) !== null && _d !== void 0 ? _d : 0)}`,
            ];
            messages.push(oiMessage.join("  "));
        }
        const wave = projection.waves[timeframe];
        if (wave) {
            const blocks = (wave.exit_time - wave.enter_time) / 1000 / timeframe;
            const prc = toMeasurePrice(wave.enter_price, wave.exit_price).toFixed(2);
            messages.push([
    ...
    return (_jsx(Stack, { children: messages.map((messages, idx) => (_jsx(Typography, { variant: "caption", children: messages }, idx))) }));
}
