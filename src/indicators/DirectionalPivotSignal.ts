...
class SignalLib {
  ...
  private historyHrCond: boolean[] = [];
  private historyLrCond: boolean[] = [];
  private historyDirCond: boolean[] = [];

  // Previous raw expressions (before the `[1]` shift)
  private prevExprHigh: boolean | null = null;
  private prevExprLow: boolean | null = null;

  constructor(
    depth: number,
    deviation: number,
    backstep: number,
    mintick: number = 0.00001
  ) {
    this.depth = depth;
  ...
  private highestIndex(data: number[], depth: number): number {
  ...
  private lowestIndex(data: number[], depth: number): number {
    if (data.length === 0) return 0;
    const start = Math.max(0, data.length - depth);
    let idx = start;
    let min = Infinity;
  ...
  next(kLineData: KLineData, index: number): void {
  ...
  private barSinceTrue(history: boolean[]): number {
  ...
  getState(): DirectionalPivotValue {
    return {
...
registerIndicator<DirectionalPivotValue, number>({
  name: "DirectionalPivotSignal",
  calcParams: [12, 5, 2],
  precision: 12,
  calc: (
    kLineDataList: KLineData[],
    indicator: Indicator<DirectionalPivotValue, number>
  ) => {
    const [depth, deviation, backstep] = indicator.calcParams as number[];
    const engine = new SignalLib(depth, deviation, backstep);

    const result: DirectionalPivotValue[] = [];
  ...
  draw: ({ ctx, indicator, xAxis, yAxis, chart }) => {
    ...
    for (let i = 1; i < result.length; i++) {
      ...
      if (current.direction !== previous.direction && current.zee2) {
        ...
        const index = current.zee2Index ?? i;
