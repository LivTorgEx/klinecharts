...
export class ImbalanceIndicator {
  ...
  private priceHistory: KLineData[];
  private imbalances: Imbalance[];

  constructor(private maxImbalances: number = 10) {
    this.priceHistory = [];
    this.imbalances = [];
  }
  ...
  next(input: KLineData) {
    ...
    let imbalance: Imbalance | undefined = undefined;
    ...
    this.imbalances = this.imbalances.filter((imbalance) =>
      imbalance.type === "Bearish"
...
registerIndicator({
  ...
  calc: (kLineDataList: KLineData[]): ImbalanceOutput[] => {
    ...
    const blocks: ImbalanceOutput[] = [];
    kLineDataList.map((kLineData, idx) => {
      const block = imbalances.find(
        (im) => im.timestamp === kLineData.timestamp
      );
      if (block) {
        blocks.push({
          index: idx,
