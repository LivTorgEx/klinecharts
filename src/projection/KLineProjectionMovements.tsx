...
function updateMovs(
  mov: SuggestionLineConsolidationMove,
  acc: SuggestionLineConsolidationMove[]
) {
  if (acc.length === 0) {
    acc.push(mov);
    return;
  ...
  const existingIndex = acc.findIndex(
    (m) => m.status === mov.status && m.time_start === mov.time_start
  );
  if (existingIndex !== -1) {
  ...
  const handleUpdateProjection = useCallback(
    (event: WebsocketProjectionEvent) => {
      ...
      movs.current.forEach((mov) => {
        ...
        const priceEnd =
          mov.direction === "SHORT" ? mov.price_min : mov.price_max;
